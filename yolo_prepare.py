...
def make_grid_cell_data(data):
    ...
    grid_height = img_height / S
    grid_width = img_width / S

    grid_x_index = int(math.floor(x_center / grid_width))
    grid_y_index = int(math.floor(y_center / grid_height))

    grid_x = grid_x_index * grid_width
    grid_y = grid_y_index * grid_height

    x_center_relative = (x_center - grid_x ) / grid_width
    y_center_relative = (y_center - grid_y ) / grid_height

    grid_data = np.zeros((S, S, 5))
...
yolo_metadata = np.zeros((len(dogs_metadata), S, S, 5))
