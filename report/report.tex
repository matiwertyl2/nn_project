...
We optimize the weighted mean least squares error, as suggested in the paper \cite{yolo} \\
...
As a feature extractor we tried to use pretrained vgg and resnet50 networks. On top of that we added 2 fully connected layers and ReLU for nonlinerarity. For regularization we used drouput with $p = 0.5$. 
...
We managed to train the network down to 25\% train error rate. However the test error rate has never been smaller than 65\%. We tried different configurations of hyperparameters, yet it did not improve the results. We decided to abandon the YOLO approach and move to the YOLO 9000.
